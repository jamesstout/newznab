...
[ ! -f $NEWZNAB_PATH/update_binaries.php ] && echo $NEWZNAB_PATH/update_binaries.php not found && exit
[ ! -f $NEWZNAB_ADMIN_PATH/nzb-importmodified.php ] && echo $NEWZNAB_ADMIN_PATH/nzb-importmodified.php not found && exit
[ ! -f $NEWZNAB_PATH/backfill.php ] && echo $NEWZNAB_PATH/backfill.php not found && exit
...
GROUPCOUNT=`$MYSQL -u$MyUSER --password=$MyPASS $DATABASE -e "$MYSQL_CMD4" | grep -o [0-9]*`
printf "\033]0; Loop $LOOP - Running $PHP $NEWZNAB_PATH/backfill.php on $GROUPCOUNT groups\007\003\n"
